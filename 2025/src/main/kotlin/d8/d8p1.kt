...
fun main() {
    ...
    val toConnect = distances.toList().sortedBy { it.second }.take(1000).map{it.first}
    ...
    for (connection in toConnect) {
        val allAffected = connection.flatMap{ groupMemberships[it]!! }

        for (affected in allAffected) groupMemberships[affected] = allAffected.toSet()
    }

    val x = groupMemberships.values.toSet().sortedBy { it.size }.reversed().take(3).fold( 1, {a, b -> a * b.size})
    println(x)
}